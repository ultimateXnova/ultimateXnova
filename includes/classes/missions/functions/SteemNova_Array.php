...
{
	$attack = array(['attack' => 0, 'shield' => 0])[0];
	$defense = array(['attack' => 0, 'shield' => 0])[0];
	// attackers shoot
	...
	return array(['attack' => $attack['attack'], 'defense' => $defense['attack'], 'attackShield' => $defense['shield'], 'defShield' => $attack['shield']])[0];
}
	...
	foreach ($attackers as $fleetID => &$attacker)
	{
		...
		for ($i = 0; $i < count($attacker['units']); $i++)
		{
			...
			if ($unit['armor'] <= 0)
			{
				...
				unset($attacker['units'][$i]);
				$attacker['units'] = array_values($attacker['units']);
				$i--;
			}
			else
			{
...
function initCombatValues(&$fleets, $firstInit = false)
{
	...
	foreach ($fleets as $fleetID => $attacker)
	{
		...
		if ($firstInit)
		{
			...
			$fleets[$fleetID]['units'] = array(); // array();
		}
