...
<div id="buildlist" class="infos1">
		...
		<div class="buildb">
				...
				{if $List@first}
				<div id="progressbar" data-time="{$List.resttime}"></div></div>
			<div class="bulida">
				...
				{/if}
				<br><span style="color: #319238" data-time="{$List.endtime}" class="timer">{$List.display}</span>
			</div>
	{/foreach}
	...
	<div class="buildl">
					...
					{else}
						{if $RoomIsOk}
							...
							{else}
							<span style="color:#826121">{if $Element.level == 0 && $Element.levelToBuild == 0}{$LNG.bd_build}{else}{$LNG.bd_build_next_level}{$Element.levelToBuild + 1}{/if}</span>
							{/if}
						{else}
						<span style="color:#826121">{$LNG.bd_no_more_fields}</span>
						{/if}
					{/if}
							...
							{if ($ID == 44 && !$HaveMissiles) ||  $ID != 44}<br><a class="tooltip_sticky" data-tooltip-content="
								...
								<table style='width:300px'>
									...
									<tr>
										...
										<td><span style='color:{if empty($Element.destroyOverflow[$RessID])}#319238{else}#826121{/if}'>{$ResCount|number}</span></td>
									</tr>
									{/foreach}
									<tr>
...
<span>{foreach $Element.costResources as $RessID => $RessAmount}
					...
					{if $Element.maxLevel == $Element.levelToBuild}
						<span style="color:#826121">{$LNG.bd_maxlevel}</span>
					{elseif ($isBusy.research && ($ID == 6 || $ID == 31)) || ($isBusy.shipyard && ($ID == 15 || $ID == 21))}
						<span style="color:#826121">{$LNG.bd_working}</span>
					{else}
						{if $RoomIsOk}
